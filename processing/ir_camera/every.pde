...
class Every {
  ...
  // everthing public
  public int last; // last time we fired
  public int interval; //  = 1000; // "delay" till next firing
  ...
  Every(int interval, boolean now) { 
  ...
  boolean now() {
    ...
    int now = millis(); // minimize drift due to this fn
    int diff = now - last;

    if (diff >= interval) {
      int drift = diff % interval;
  ...
  boolean now(int x_interval) {
    int now = millis(); // minimize drift due to this fn
    int diff = now - last;

    if (diff >= x_interval) {
      int drift = diff % x_interval;
      //Serial << "drift " << last << " now " << now << " d: " << drift << endl;
      last = now;
      last -= drift;
      return true;
  ...
  void reset(boolean now) {
    last = millis();
    if (now) last -= interval;
  }
  void reset(int interval) { 
    reset(interval, false);
  }
  void reset(int interval, boolean now) { 
    this.interval = interval; 
    reset(now);
  }
...
/*
 ...
 Toggle(boolean now = false) : Every(now) {}
 Toggle(int interval, boolean now = false) : Every( (int) interval, now) {}
 Toggle(int interval, boolean now = false) : Every( interval, now ) {}
 ...
 * Yikes, don't know how to receive a lambda with args, the base class's matches before we do
 ...
 template <typename T>
 boolean operator()(T lambdaF(const boolean state) ) {
...
/*
 ...
 Pattern(const unsigned int seq_count, const int pattern[], boolean now = false)
 : 
 Every {
 pattern[0], now
 }
 , seq_count(seq_count), _pattern(pattern)
 {
 }
 
 // for default heartbeat
 Pattern(boolean now = false)
 : 
 Every(1000, now)
 { 
 const static int _heartbeat[] = {150, 250, 150, 700}; // heart
 ...
 EveryCount(int interval, int count, boolean now = false)
 : 
 Every {
 interval, now
 }
 ...
 ::  using Everyoperator(); // in every subclass if you add a ()
 ...
 * Yikes, don't know how to receive a lambda with args, the base class's matches before we do
 ...
 Every2Sequence(unsigned int interval, const int seq_count, const T sequence[], boolean now = false)
 : 
 Every {
 interval, now
 }
 , seq_count(seq_count), _sequence(sequence)
 {
 }
 
 // for toggles, it's just:
 Every2Sequence(unsigned int interval, boolean now = false)
 : 
 Every {
 interval, now
 }
 , seq_count(2), _sequence(_toggle)
 {
 }
 ...
 Timer(int interval) : 
 last(millis()), running(true), interval(interval) {
 }
 ...
 ::    Timeroperator()(); // to update running;
 return ! running;
 }
 template <typename T>
 ...
 public:
 // everthing public
 int count = 1; // number of times to fire
 ...
 public:
 // everthing public
 int original_count = 1; // fire on nth
 int count = 1; // count till next fire
 
 NthTime(int n) : 
 original_count(n), count(n) {
 }
